...
const professionals = [
  ...
  { name: "يزن الحداد", trade: "نجار أثاث وديكور", location: "عمّان، خلدا", rating: "5.0", reviews: 24, image: "assets/mihnati-craftsman-profile.webp", verified: true, tag: "الأعلى تقييماً" },
...
const platformBenefits = [
  { icon: ShieldCheck, number: "01", title: "ثقة موثّقة", text: "شهادات وتقييمات حقيقية تساعدك تختار براحة بال." },
  { icon: Sparkles, number: "02", title: "ملف يحكي عنك", text: "فيديو 15 ثانية وبطاقة QR تعرض مهارتك أينما كنت." },
  { icon: MessageCircle, number: "03", title: "اتفاق أسرع", text: "اطلب عرض سعر واحد واستقبل عروضًا واضحة من حولك." },
];
...
export default function Home() {
  ...
  const filteredPros = useMemo(() => {
    ...
    return professionals.filter((professional) => `${professional.name} ${professional.trade} ${professional.location}`.includes(normalized));
  }, [query]);

  const handleSearch = (event: FormEvent) => {
    event.preventDefault();
    const selectedTrade = trade === "كل المهن" ? query || "مهني موثوق" : trade;
    toast.success(`نبحث لك عن ${selectedTrade} في ${city}`, { description: "عرضنا لك أقرب النتائج الموثوقة." });
  ...
  return (
    <div dir="rtl" className="min-h-screen overflow-hidden bg-[#fbfaf7] text-[#102d5e]">
      <div className="hidden bg-[#102d5e] text-white sm:block">
        <div className="container flex h-9 items-center justify-between text-[11px] font-semibold text-white/80">
          <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#f1a262]" /> منصة الثقة للمهنيين والعملاء في الأردن</span>
          <button onClick={() => actionToast("سجل اهتمامك وسنخبرك عند الإطلاق")} className="flex items-center gap-1 text-[#f1a262] hover:text-white">للمهنيين: انضم مجاناً <ArrowLeft size={13} /></button>
        </div>
      </div>
      <header className="relative z-50 border-b border-[#102d5e]/10 bg-[#fbfaf7]/90 backdrop-blur-xl">
        <div className="container flex h-[76px] items-center justify-between gap-5">
          ...
          <nav className="hidden items-center gap-7 text-[14px] font-semibold text-[#304b71] lg:flex">
            ...
            <a className="transition-colors hover:text-[#dc6b1b]" href="#features">كيف تعمل</a>
          ...
          <button onClick={() => setMobileNav((value) => !value)} className="rounded-xl p-2 text-[#102d5e] sm:hidden" aria-label="فتح القائمة">{mobileNav ? <X /> : <Menu />}</button>
        </div>
        {mobileNav && <div className="border-t border-[#102d5e]/10 bg-white px-4 py-4 sm:hidden"><div className="container flex flex-col gap-4 text-sm font-bold"><a href="#professionals" onClick={() => setMobileNav(false)}>اعثر على مهني</a><a href="#work" onClick={() => setMobileNav(false)}>معرض الأعمال</a><a href="#features" onClick={() => setMobileNav(false)}>كيف تعمل</a><a href="#market" onClick={() => setMobileNav(false)}>سوق الأدوات</a><button onClick={() => actionToast("نموذج التسجيل سيجهز ملفك خلال دقائق")} className="rounded-xl bg-[#dc6b1b] px-4 py-3 text-white">سجّل كمهني</button></div></div>}
      </header>

      <main id="top">
        <section className="noise hero-grid relative border-b border-[#102d5e]/10 bg-[#f4f7fb]">
          <div className="container relative grid min-h-[620px] items-center gap-10 py-14 lg:grid-cols-[1fr_1.1fr] lg:gap-16 lg:py-20">
            <div className="relative z-10 max-w-[620px] text-right">
              ...
              <form onSubmit={handleSearch} className="fade-up fade-up-delay-3 mt-9 rounded-[22px] border border-[#102d5e]/10 bg-white p-2 shadow-[0_20px_60px_rgba(16,45,94,.14)] sm:max-w-[630px]">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  <div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl bg-[#f7f8fa] px-4 py-3.5"><Search className="shrink-0 text-[#dc6b1b]" size={20} /><input value={query} onChange={(event) => { setQuery(event.target.value); setTrade("كل المهن"); }} placeholder="ماذا تحتاج؟ (كهربائي، نجار...)" className="min-w-0 w-full bg-transparent text-sm font-semibold text-[#102d5e] outline-none placeholder:text-[#8290a4]" /></div>
                  <div className="flex items-center gap-2 rounded-2xl bg-[#f7f8fa] px-4 py-3.5 sm:w-[148px]"><MapPin className="shrink-0 text-[#dc6b1b]" size={18} /><select value={city} onChange={(event) => setCity(event.target.value)} className="w-full appearance-none bg-transparent text-sm font-semibold text-[#304b71] outline-none"><option>عمّان</option><option>إربد</option><option>الزرقاء</option><option>العقبة</option></select><ChevronDown size={15} className="pointer-events-none -mr-5 text-[#8998ad]" /></div>
            ...
            <div className="relative hidden h-[470px] lg:block">
              <div className="absolute right-[3%] top-2 h-[432px] w-[88%] overflow-hidden rounded-[34px] border-[10px] border-white bg-[#dce8f4] shadow-[0_30px_70px_rgba(16,45,94,.18)]"><img src="assets/mihnati-hero-workshop.webp" alt="مهني يعمل في ورشة" className="h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#102d5e]/60 via-transparent to-transparent" /><div className="absolute bottom-6 right-6 left-6 flex items-end justify-between text-white"><div><div className="mb-1 text-xs font-bold text-white/80">مهارات حقيقية. فرص حقيقية.</div><div className="display-font text-2xl font-extrabold">خلّي شغلك يحكي عنك</div></div><button onClick={() => actionToast("معرض الأعمال القصيرة قريباً")} className="grid h-11 w-11 place-items-center rounded-full bg-[#dc6b1b] shadow-xl" aria-label="شاهد معرض الأعمال"><Play size={18} fill="currentColor" /></button></div></div>
        ...
        <section className="border-b border-[#102d5e]/8 bg-white"><div className="container grid grid-cols-2 divide-x divide-x-reverse divide-[#102d5e]/10 py-7 sm:grid-cols-4"><Stat value="2,500+" label="مهني موثّق" /><Stat value="4.9/5" label="متوسط التقييم" /><Stat value="12" label="محافظة مغطاة" /><Stat value="60 دقيقة" label="متوسط الرد" /></div></section>
        ...
        <section className="container py-20 sm:py-24" id="categories">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {categories.map(({ label, count, icon: Icon, tone }) => <button key={label} onClick={() => { setActiveCategory(label); setQuery(label); setTrade(label); toast.success(`تم اختيار تخصص ${label}`); document.getElementById("professionals")?.scrollIntoView({ behavior: "smooth" }); }} className={`lift group rounded-2xl border p-4 text-right ${activeCategory === label ? "border-[#dc6b1b] bg-white shadow-md" : "border-[#102d5e]/8 bg-white/60"}`}><div className={`mb-6 grid h-11 w-11 place-items-center rounded-xl ${tone}`}><Icon size={21} /></div><div className="mb-1 text-sm font-extrabold text-[#102d5e]">{label}</div><div className="text-xs font-medium text-[#8190a5]">{count}</div></button>)}
          </div>
        </section>
        ...
        <section id="professionals" className="bg-[#f0f4f8] py-20 sm:py-24">
          <div className="container"><SectionHeading eyebrow="موصى بهم لك" title="مهنيون تثق في شغلهم" link="عرض الجميع" />
            <div className="grid gap-5 lg:grid-cols-3">
              {filteredPros.map((professional, index) => <article key={professional.name} className="lift overflow-hidden rounded-[22px] border border-[#102d5e]/8 bg-white shadow-[0_8px_26px_rgba(16,45,94,.05)]"><div className="relative h-56 overflow-hidden"><img src={professional.image} alt={professional.trade} className="h-full w-full object-cover transition duration-500 hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#102d5e]/55 to-transparent" /><span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-bold text-[#1b8c74] shadow-sm"><span className="ml-1 inline-block h-1.5 w-1.5 rounded-full bg-[#1b8c74]" />{professional.tag}</span><button onClick={() => setLiked((old) => old.includes(index) ? old.filter((item) => item !== index) : [...old, index])} className="absolute left-4 top-4 grid h-9 w-9 place-items-center rounded-full bg-white/90 text-[#8090a5] shadow-sm hover:text-[#d65959]" aria-label="إضافة للمفضلة"><Heart size={17} className={liked.includes(index) ? "fill-[#d65959] text-[#d65959]" : ""} /></button><div className="absolute bottom-4 right-4 text-white"><div className="flex items-center gap-1.5 text-xs font-semibold"><MapPin size={13} /> {professional.location}</div></div></div><div className="p-5"><div className="mb-3 flex items-start justify-between gap-2"><div><h3 className="display-font text-lg font-extrabold text-[#102d5e]">{professional.name} <BadgeCheck className="mr-1 inline text-[#258cb7]" size={16} fill="currentColor" color="white" /></h3><p className="mt-1 text-sm font-medium text-[#71829c]">{professional.trade}</p></div><div className="flex items-center gap-1 rounded-lg bg-[#fff5dc] px-2 py-1 text-xs font-extrabold text-[#a56b00]"><Star size={13} fill="currentColor" /> {professional.rating}</div></div><div className="flex items-center justify-between border-t border-[#102d5e]/8 pt-4 text-xs font-semibold text-[#8492a5]"><span>{professional.reviews} تقييماً موثوقاً</span><button onClick={() => actionToast(`فتح ملف ${professional.name}`)} className="flex items-center gap-1 font-bold text-[#254d88]">عرض الملف <ArrowLeft size={14} /></button></div></div></article>)}
            </div>
